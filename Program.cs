...
var hub = new ChatHub();
app.MapGet("/", () => Results.Ok(new { name = "SigmaChat server", status = "online", storage = "ephemeral" }));
...
sealed class ChatHub
{
            ...
            while (ws.State == WebSocketState.Open && !ct.IsCancellationRequested)
            {
                ...
                if (msg.Type == "chat")
                {
                    ...
                    if (text.Length is > 0 and <= 1000)
                        await Broadcast(room, new { type = "chat", id = Guid.NewGuid().ToString("N")[..10], senderId = member.Id, sender = member.Name, message = text, timestamp = DateTimeOffset.UtcNow }, ct);
                }
            }
        }
    ...
    {
        var buffer = new byte[4096]; var result = await ws.ReceiveAsync(buffer, ct);
        if (result.MessageType == WebSocketMessageType.Close || !result.EndOfMessage) return null;
        try { return JsonSerializer.Deserialize<Incoming>(buffer.AsSpan(0, result.Count), json); } catch { return null; }
    }
...
sealed class Room { public object Gate { get; } = new(); public ConcurrentDictionary<string, Member> Members { get; } = new(); }
sealed record Member(string Id, string Name, WebSocket Socket);
sealed class Incoming { public string? Type { get; set; } public string? Room { get; set; } public string? Name { get; set; } public string? Message { get; set; } }
